    ...
    <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="store">
        <div class="flex flex-wrap -mb-8 -mr-6 p-8">
          ...
          <select-input v-model="form.organization_id" :error="form.errors.roleIds" class="pb-8 pr-6 w-full lg:w-1/2" label="Organization">
            <option v-for="organization in organizations" :key="organization.id">{{ organization.name }}</option>
          </select-input>

          <select-input multiple v-model="form.roleIds" :error="form.errors.roleIds" class="pb-8 pr-6 w-full lg:w-1/2" label="Roles">
            <option v-for="role in roles" :key="roles.id">{{ role.name }}</option>
          </select-input>
